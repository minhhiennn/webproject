				...
				<div class="col-md-9 col-sm-8">
										...
										<div class="result-short">
											...
											<div class="result-short-selection">
												<select onchange="location = this.value;">
													...
													<option
														value="http://localhost:8080/WebProject/FilterSortServlet?productType=<%=productType%>&sortcode=AtoZ&nhom=kids&title=<%=title%>&amount=<%=amount%>&index=1">Sắp
														xếp theo tên từ A đến Z</option>
													<option
														value="http://localhost:8080/WebProject/FilterSortServlet?productType=<%=productType%>&sortcode=ZtoA&nhom=kids&title=<%=title%>&amount=<%=amount%>&index=1">Sắp
														xếp theo tên từ Z đến A</option>
													<option
														value="http://localhost:8080/WebProject/FilterSortServlet?productType=<%=productType%>&sortcode=lowtohigh&nhom=kids&title=<%=title%>&amount=<%=amount%>&index=1">Sắp
														xếp theo giá thấp đến cao</option>
													<option
														value="http://localhost:8080/WebProject/FilterSortServlet?productType=<%=productType%>&sortcode=hightolow&nhom=kids&title=<%=title%>&amount=<%=amount%>&index=1">Sắp
					...
					<div class="shop-pagination floatright">
						<ul class="pagination">
							...
							%>
							<li><a id=<%=string%>
								href="http://localhost:8080/WebProject/FilterSortServlet?index=<%=i + 1%>&amount=<%=amount%>&nhom=kids&productType=<%=productType%>&sortcode=<%=sortcode%>"><%=(i + 1)%></a></li>
							<%
								}
							%>
