				...
				<div class="col-md-9 col-sm-8">
								...
								<div class="product-item-heading">
									...
									<div class="result-short-view">
										<div class="result-short">
											...
											<%
												String searchProduct = (String) request.getAttribute("search");
												String selectform = (String) request.getAttribute("select");
											%>
											<div class="result-short-selection">
												<select onchange="location = this.value;">
													<option value="">Mặc định phân loại</option>
													<option
														value="http://localhost:8080/WebProject/SearchSortServlet?sortcode=AtoZ&index=1&search=<%=searchProduct%>&select=<%=selectform%>">Sắp
														xếp theo tên từ A đến Z</option>
													<option
														value="http://localhost:8080/WebProject/SearchSortServlet?sortcode=ZtoA&index=1&search=<%=searchProduct%>&select=<%=selectform%>">Sắp
														xếp theo tên từ Z đến A</option>
													<option
														value="http://localhost:8080/WebProject/SearchSortServlet?sortcode=lowtohigh&index=1&search=<%=searchProduct%>&select=<%=selectform%>">Sắp
														xếp theo giá thấp đến cao</option>
													<option
														value="http://localhost:8080/WebProject/SearchSortServlet?sortcode=hightolow&index=1&search=<%=searchProduct%>&select=<%=selectform%>">Sắp
								...
								<div class="single-item-area">
									<div class="single-item">
										<div class="product-item-img">
											<a href="http://localhost:8080/WebProject/SingleProductServlet?productID=<%=list.get(i).getID()%>&index=1"> <img class="primary-img" src=<%=listImg.get(0).getFileImg()%>
												alt="item" style="width: 264px; height: 180px;"> <img
												class="secondary-img" src=<%=listImg.get(1).getFileImg()%> alt="item" style="width: 264px; height: 180px;">
											</a>
					...
					<div class="shop-pagination floatright">
						<ul class="pagination">
							...
							%>
							<li><a id=<%=string%>
								href="http://localhost:8080/WebProject/ProductServlet?index=<%=i + 1%>&search=<%=searchProduct%>&select=<%=selectform%>"><%=(i + 1)%></a></li>
							<%
								}
							%>
